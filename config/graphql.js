...
function parseDestinationId(destinationId) {
  if (typeof destinationId !== "string") {
    throw new TypeError("destinationId must be a string");
  }

  const [ns, id] = destinationId.split("-").map(Number);

  if (!Number.isInteger(ns) || !Number.isInteger(id)) {
    throw new Error(`Invalid destinationId: ${destinationId}`);
  }
    ...
    variables: {
      ...
      params: {
        ...
        searchExecutionContext: {
          searchType: "MULTI_POLL_WITH_DEALS",
        },
...
export function buildAccommodationDealsPayload({
  ...
  if (!Number.isInteger(numericAccommodationId)) {
    throw new Error(`Invalid accommodationId: ${accommodationId}`);
  }
